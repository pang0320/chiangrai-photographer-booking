...
<section class="mx-auto max-w-md px-4 py-12">
    <div class="rounded-3xl bg-white p-8 shadow-xl ring-1 ring-slate-200">
        ...
        <form method="post" class="mt-6 grid gap-4">
            ...
            <button class="btn-cta btn-lg rounded-2xl"><i class="fa-solid fa-key mr-2"></i>สร้างลิงก์รีเซ็ต</button>
        </form>
        <?php if ($resetLink): ?>
            <p class="mt-5 break-all rounded-2xl bg-slate-50 p-4 text-sm">
                Development reset:
                <?= clean_context_button('/reset_password.php', ['token' => $resetLink], '<i class="fa-solid fa-key mr-2"></i>เปิดหน้าตั้งรหัสผ่านใหม่', 'btn-primary btn-md mt-3') ?>
            </p>
        <?php endif; ?>
    </div>
</section>
