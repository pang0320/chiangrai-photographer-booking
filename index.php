...
<section class="hero-frame relative overflow-hidden bg-neutral-950 text-white">
    <div class="absolute inset-0">
        <img class="h-full w-full object-cover opacity-45" src="https://images.unsplash.com/photo-1511285560929-80b456fea0bc?auto=format&fit=crop&w=2200&q=85" alt="">
...
<section class="marketplace-band py-16">
    <div class="stock-shell px-4 sm:px-6 lg:px-8">
        <div class="flex flex-wrap items-end justify-between gap-5">
            <div>
                <p class="section-kicker">Featured Photographers</p>
...
<section class="stock-shell px-4 py-16 sm:px-6 lg:px-8">
    <div class="grid gap-8 lg:grid-cols-[360px_1fr] lg:items-start">
        <div class="lg:sticky lg:top-24">
            <p class="section-kicker">Top Rated</p>
        ...
        <div class="flex gap-5 overflow-x-auto pb-4">
            <?php foreach ($topRated as $p): ?>
                ...
                <a href="/photographer_detail.php?id=<?= (int)$p['id'] ?>" class="stock-card stock-card-hover w-80 shrink-0 rounded-[1.75rem] p-4">
                    <img class="h-48 w-full rounded-[1.35rem] object-cover" src="<?= h(public_image($topRatedImage, 'https://images.unsplash.com/photo-1492691527719-9d1e07e534b4?auto=format&fit=crop&w=900&q=80')) ?>" alt="">
                    <div class="p-2">
...
<section class="bg-neutral-950 py-16 text-white">
    <div class="stock-shell grid gap-10 px-4 sm:px-6 lg:grid-cols-[.9fr_1.1fr] lg:px-8">
        ...
        <div class="grid gap-4">
            <?php foreach ([['fa-magnifying-glass','ค้นหาช่างภาพ','เลือกอำเภอ ประเภทงาน วันที่ คะแนน และงบประมาณ'],['fa-images','ดูผลงาน','ดู Portfolio รีวิว พื้นที่ให้บริการ และวันว่าง'],['fa-paper-plane','ส่งคำขอจอง','กรอกฟอร์มสั้น ๆ พร้อมรายละเอียดงาน'],['fa-comments','ติดต่อโดยตรง','โทร LINE Facebook Instagram หรือ Website ของช่างภาพ']] as $step): ?>
...
<section class="marketplace-band py-16">
    <div class="stock-shell px-4 sm:px-6 lg:px-8">
        <div class="flex flex-wrap items-end justify-between gap-5">
            <div>
                <p class="section-kicker">Portfolio Showcase</p>
        ...
        <div class="masonry-gallery mt-8">
            <?php foreach ($portfolioShowcase as $item): ?>
                <a href="/photographer_detail.php?id=<?= (int)$item['photographer_id'] ?>" class="media-tile block rounded-[1.5rem] shadow-xl">
                    <img class="min-h-[220px]" src="<?= h(public_image($item['image_path'], 'https://images.unsplash.com/photo-1516035069371-29a1b244cc32?auto=format&fit=crop&w=900&q=85')) ?>" alt="">
...
<section class="stock-shell grid gap-8 px-4 py-16 sm:px-6 lg:grid-cols-[1.1fr_.9fr] lg:px-8">
            ...
            <?php foreach ($reviews as $review): ?>
                <article class="stock-card rounded-[1.5rem] p-6">
                    <div class="flex items-center gap-3">
                        <img class="h-12 w-12 rounded-2xl object-cover" src="<?= h(public_image($review['avatar'], 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=300&q=80')) ?>" alt="">
                        <div>
    ...
    <div class="dashboard-hero rounded-[2rem] p-8 text-white">
        ...
        <p class="mt-4 leading-8 text-white/68">สร้างโปรไฟล์ อัปโหลด Portfolio กำหนดพื้นที่ วันว่าง และรับคำขอจองผ่านระบบเดียว</p>
        ...
        <div class="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            <?php foreach ($articles as $article): ?>
                <article class="stock-card stock-card-hover rounded-[1.75rem]">
                    <img class="h-48 w-full object-cover" src="<?= h(public_image($article['cover_image'], 'https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?auto=format&fit=crop&w=900&q=80')) ?>" alt="">
                    <div class="p-6">
...
<section class="stock-shell px-4 py-16 sm:px-6 lg:px-8">
    <div class="grid gap-8 lg:grid-cols-[360px_1fr]">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <?php foreach ([['เว็บไซต์รับชำระเงินไหม','ไม่รับชำระเงิน เว็บไซต์เป็นเพียงตัวกลางค้นหาและติดต่อช่างภาพเท่านั้น'],['ติดต่อช่างภาพอย่างไร','กดโทร LINE Facebook Instagram หรือ Website ในหน้าโปรไฟล์ช่างภาพ'],['รีวิวได้เมื่อไหร่','ลูกค้ารีวิวได้เฉพาะ booking ที่สถานะ completed และ 1 booking รีวิวได้ 1 ครั้ง'],['ถ้าไม่พบช่างภาพในพื้นที่ทำอย่างไร','ระบบจะแนะนำช่างภาพอำเภอใกล้เคียงโดยคำนวณจาก latitude/longitude']] as $faq): ?>
