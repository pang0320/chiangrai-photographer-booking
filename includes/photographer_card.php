...
<article class="stock-card stock-card-hover rounded-[1.75rem]">
    <a href="/photographer_detail.php?id=<?= (int)$p['id'] ?>" class="media-tile block h-64">
        <img src="<?= h(public_image($cardImagePath, 'https://images.unsplash.com/photo-1516035069371-29a1b244cc32?auto=format&fit=crop&w=900&q=80')) ?>" alt="">
    ...
    <div class="p-5">
        ...
        <div class="mt-5 grid grid-cols-3 gap-3">
            <a class="rounded-full border border-neutral-200 px-4 py-2.5 text-center text-sm font-black hover:border-neutral-950 hover:bg-neutral-950 hover:text-white" href="/photographer_detail.php?id=<?= (int)$p['id'] ?>"><i class="fa-solid fa-eye mr-1"></i>ดูโปรไฟล์</a>
            <a class="stock-button rounded-full px-4 py-2.5 text-center text-sm font-black" href="/customer/create_booking.php?photographer_id=<?= (int)$p['id'] ?>"><i class="fa-solid fa-calendar-check mr-1"></i>ส่งคำขอจอง</a>
            <a class="rounded-full border border-neutral-200 px-4 py-2.5 text-center text-sm font-black hover:border-neutral-950 hover:bg-neutral-950 hover:text-white" href="/compare.php?ids=<?= (int)$p['id'] ?>"><i class="fa-solid fa-code-compare mr-1"></i>เทียบ</a>
        </div>
    </div>
</article>
