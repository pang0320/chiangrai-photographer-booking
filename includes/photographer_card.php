...
<article class="stock-card stock-card-hover rounded-[1.75rem]" data-clickable-card data-card-form="<?= h($cardFormId) ?>" tabindex="0" role="link" aria-label="ดูโปรไฟล์ <?= h($cardName) ?>">
    ...
    <div class="p-5">
        ...
        <div class="mt-5 grid gap-3 sm:grid-cols-3">
            <?= clean_context_button('/photographer_detail.php', $cardDetailParams, '<i class="fa-solid fa-eye mr-1"></i>ดูโปรไฟล์', 'btn-primary btn-sm w-full text-center') ?>
            <?= clean_context_button('/customer/create_booking.php', ['photographer_id' => $cardId], '<i class="fa-solid fa-calendar-check mr-1"></i>จอง', 'btn-cta btn-sm w-full text-center') ?>
            <?= clean_context_button('/compare.php', ['ids' => $cardId], '<i class="fa-solid fa-code-compare mr-1"></i>เทียบ', 'btn-muted btn-sm w-full text-center') ?>
        </div>
    </div>
</article>
